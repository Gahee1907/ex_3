...
for(let el of lists) {
    ...
    play.addEventListener("click", e=>{
        ...
        //play 버튼부처 audio 요소까지 탐색한 뒤 음악 재생
...
prev.addEventListener("click", ()=>{
    ...
    //현재 패널의 순번이 0이면 다시 마지막 패널의 순번으로 변경하고 그렇지 않으면 현재 패널 순번에서 1씩 감소시켜서 activation 함수 호출
...
next.addEventListener("click", ()=>{
    ...
    //현재 패널의 순번이 마지막 순번이면 다시 처음 패널의 순번으로 변경하고 그렇지 않으면 현재 패널 순번에서 1씩 증가시켜서 activation함수 호출
...
next.addEventListener("click", ()=>{
    ...
    initMusic();
    num--;
    TransformStream.style.transform = `ratate(${deg * num}deg)`;
...
pause.addEventListener("click", e=>{
    let isActive = e.currentTarget.closest("acticle").classList.contains("on");
    if(isActive){
